...
## 1.1 Capability classification (Keep / Refactor / Build-new)

Grounded in the verified repo. **Key reveal: most of the HR *domain* already exists as
disconnected table shells** (an earlier "employee-centric HR" sprint left them). The genuinely
*new* work is the **kernel hardening** (global sequence, RLS), the **access/identity layer**
(auth, RBAC, access log, sessions), the **integrity layer** (snapshots, workflows, encryption),
plus Leave, Compliance modules, and self-service. This shifts effort from "build" to "refactor + wire".

| Capability | Classification | Note (verified) |
|---|---|---|
| Event store (`orgEventsTable`, transactional `appendDomainEvent`) | **Keep** | events-as-truth, single-tx append |
| Outbox + reliability (dead-letters, delivery receipts) | **Keep** | `OutboxReliabilityService` |
| Projection builder + integrity/replay | **Keep** | replay hash-compare exists |
| Per-aggregate version / OCC | **Keep** | `aggregateVersionsTable` |
| Idempotency | **Keep** | unique `(orgId, idempotencyKey)` |
| Correlation / causation IDs | **Keep** | already on events (workflow foundation) |
| Payload hashing (determinism) | **Keep** | `payloadHash` + replay compare |
| Tenant scoping column | **Keep** | `organizationId`/`orgId` everywhere = `company_id` |
| **Global ordering / sequence** | **Refactor** | today `occurredAt→aggregateId→version`; add per-company `global_sequence_number`, replace timestamp sort |
| Organization model | **Refactor** | → Company (+ jurisdiction, modules, currency, configs) |
| Users (identity shell) | **Refactor** | extend for real auth identity + status |
| Org memberships | **Refactor** | add role (RBAC) |
| Position model | **Refactor** | + grade, dept, location, budgeted_salary, occupancy |
| People → Employee (`peopleTable` exists) | **Refactor** | + status machine, payroll/bank fields, workflow_id |
| PositionAssignment (`personPositionAssignmentsTable`) | **Refactor** | + workflow_id |
| Compensation (`compensationRecordsTable`, int money) | **Refactor** | + explicit currency |
| Documents (`documentsTable` + evidence services) | **Refactor** | + lifecycle + snapshot-only templates |
| Policies (`policiesTable`) | **Refactor** | + versioning + PolicyAcknowledgement |
| Offboarding (`offboardingCompletionsTable` + service) | **Refactor** | bind to workflow + exit snapshot |
| Actor-context middleware (header-based) | **Replace** | → real auth middleware |
| `auditEventsTable` | **Refactor/Remove** | remove per §3.3, or repurpose as the access log (§9 Q6) |
| Authentication (password / OIDC / sessions) | **New** | none today |
| RBAC enforcement + Platform Super Admin | **New** | none |
| Access/Security audit log (`AccessEvent`) | **New** | none |
| Postgres RLS | **New** | not present |
| Snapshot engine (sequence-based, encrypted) | **New** | none |
| Encryption-at-rest + key rotation + crypto-shred | **New** | none |
| Workflow engine (states + failure/compensation) | **New** | none (only correlationId exists) |
| Leave system | **New** | none |
| Compliance modules (UAE) | **New** | none |
| Employee self-service | **New** | none |
| Audit / Workflow / Access-log viewers | **New** | viewers over existing/new logs |
