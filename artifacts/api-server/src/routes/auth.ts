...
import { db, pool, usersTable, sessionsTable, membershipsTable } from "@workspace/db";
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Find default company membership
    const membershipResult = await pool.query<{ company_id: string }>(
      "SELECT company_id FROM memberships WHERE user_id = $1 LIMIT 1",
      [row.id],
    );
